...
setmetatable(M, {
  __index = function(_, key)
    ...
    if key == "dev_mode" then return M.options.dev_mode end
  end,
})

---Checks if dev mode is enabled
---@return boolean
function M.is_dev_mode() return M.options.dev_mode end

---Toggle dev mode config option
---@return nil
function M.toggle_dev_mode() M.options.dev_mode = not M.options.dev_mode end

---Sets the debug mode to a specific value
---@return nil
function M.set_dev_mode(value) M.options.dev_mode = value end
...
function M.notify(message, level)
  level = level or 2
  if M.is_dev_mode() or level >= 3 then vim.schedule(function() notify(message, level) end) end
end
...
function M.debugf(msg, level, log_level, schedule)
  if M.is_dev_mode() then
    ...
    local dmsg = string.format("%s -- called in function %s (%s:%d)", msg, function_name, source, line)

    -- Output the debug message
    if not schedule then
      notify(dmsg, log_level)
    else
      vim.schedule(function() notify(dmsg, log_level) end)
    end
  end
end
