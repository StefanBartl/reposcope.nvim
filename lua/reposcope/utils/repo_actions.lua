...
--- fetch+pull sweep): this module runs one mutating git command against
--- exactly one repository, as triggered interactively (e.g. a keymap on the
--- row under the cursor in `:Reposcope status`) rather than a bulk update.
--- Each action reports success/failure via callback; the caller decides how
--- to surface it (notify, refresh a view, ...).
