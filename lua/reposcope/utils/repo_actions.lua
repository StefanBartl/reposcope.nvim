...
---@description
--- Companion to `repo_dashboard` (read-only) and `repo_updater` (directory-wide
...
--- the cursor in `:Reposcope dashboard`) rather than a bulk update. Each action
--- reports success/failure via callback; the caller decides how to surface it
--- (notify, refresh a view, ...).
---
--- Bulk is a caller's concern, not this module's: the dashboard's
...
---for a single repository so that `repo_updater` and the dashboard's
