...
---Registers the single dispatching `:Reposcope` user command. The
--- multi-repo git dashboard/update moved to gitsuite.nvim's `:Git dashboard`
--- (`:Git dashboard update` for the headless sweep) -- see that plugin's
--- `features/dashboard/` for the git-status/action panel this used to be.
local routes = build_routes()
composer.verb("Reposcope", {
  desc = "Reposcope: <subcommand> [args] (start, close, filter, prompt, ...)",
  routes = routes,
})
