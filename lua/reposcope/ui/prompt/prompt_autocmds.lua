...
function M.setup_autocmds()
  -- Manual delete+recreate on every call, not `autocmd.group()`: that helper
  -- memoizes the augroup id by name and would keep handing back the id of
  -- the group we just deleted below instead of the fresh one.
  pcall(nvim_del_augroup_by_name, "reposcope_prompt_autocmds")
  local group = nvim_create_augroup("reposcope_prompt_autocmds", { clear = true })

  -- Track Text Input
  autocmd.create("TextChangedI", function()
