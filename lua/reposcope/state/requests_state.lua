...
function M.register_request(uuid)
  if uuid and type(uuid) == "string" and uuid ~= "" then M.requests[uuid] = false end
end

---Marks a registered UUID as active
---@param uuid string
function M.start_request(uuid)
  if M.requests[uuid] == false then M.requests[uuid] = true end
end

---Marks a UUID request as completed
---@param uuid string
function M.end_request(uuid) M.requests[uuid] = nil end

---Checks if a UUID was registered
---@param uuid string
---@return boolean
function M.is_registered(uuid) return M.requests[uuid] ~= nil end

---Checks if a UUID is currently marked active
---@param uuid string
---@return boolean
function M.is_request_active(uuid) return M.requests[uuid] == true end

---Clears all tracked UUID requests
function M.clear_all_requests() M.requests = {} end
